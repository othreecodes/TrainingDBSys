								    	...
								    	<div class="col-lg-12">
								    		<form method="get" action="<s:url action='search' />" id="frmSearch" name="frmSearch">
											    <div class="input-group">
											      <input name="q" type="text" style="width:90%;z-index:0;" class="form-control search-box" placeholder="Search for...">
...
<s:form method="get" action="advanced/search!query">
	...
	<table class="TFtable table table-responsive">
		<tr>
			<td>Scope:</td>
			<td colspan="3"><s:select id="searches" onchange="javascript: __state(this.value);"
			cssClass="form-control"
				list="#{'Trainee':'Graduate Trainees','TrainingProgram':'Training Programs','Trainer':'Trainers','Organization':'Organization','TrainingProposal':'Training Proposals'}"
		...
		<tr>
			<td>Criteria:</td>
			<td colspan="3"><s:textfield name="text" value="%{text}" cssClass="form-control" cssStyle="width:100%" /><br />
		...
		<tr class="hideType">
			...
			<td>
				...
				<div class="tpType">
					...
					value="%{tpType}" multiple="true" headerKey="" headerValue=" -- " />
				</div>	
			</td>
		</tr>
		<tr class="hides">
			<td>Cost Center</td>
			<td><s:textfield name="cc" value="%{cc}" cssClass="form-control" cssStyle="width:100%" /></td>
		</tr>
		<!-- 
		<tr class="hides">
		...
		<tr class="hides">
			...
			<td><s:select
			cssClass="form-control"
				list="#{'BETWEEN':'BETWEEN','=':'EQUALS','<':'LESS THAN','<=':'LESS OR EQUAL','>':'GREATER THAN','>=':'GREATER OR EQUAL'}" name="sDate" emptyOption="true" /> 
				<iita:datepicker name="startDate" value="%{startDate}" format="dd/MM/yyyy" cssStyle="width:100px" /></td>
		</tr>
		<tr class="hides">
			<td>End Date:</td>
			<td><s:select list="#{'AND':'AND','OR':'OR'}" cssClass="form-control" name="operands" emptyOption="true" /> <s:select list="#{'=':'EQUALS','<':'LESS THAN','<=':'LESS OR EQUAL','>':'GREATER THAN','>=':'GREATER OR EQUAL'}" name="eDate" emptyOption="true" /> <iita:datepicker name="endDate" value="%{endDate}" format="dd/MM/yyyy" cssStyle="width:100px" /></td>
		</tr>
		<tr class="hides">
			<td></td>
		...
		<tr class="hides">
			...
			<td>
				...
				,'Plant Production & Health':'Plant Production & Health'}" value="%{selectedCoreCompetencies}" multiple="true" 
				 headerKey="" cssClass="form-control" headerValue="-- SELECT CORE COMPETENCIES --" />
				 
				<s:select name="hubs" list="#{'Western Africa':'Western Africa','Eastern Africa':'Eastern Africa',
				'Central Africa':'Central Africa','Southern Africa':'Southern Africa'}" cssClass="form-control" value="%{selectedHubs}" multiple="true"
				 headerKey="" headerValue="-- SELECT HUBs --" />
			</td>
		</tr>
		<tr>
			<td></td>
			<td colspan="3"><s:submit cssClass="btn btn-success" value="Search" /> 
			<s:submit
				value="Download" cssClass="btn btn-info" action="advanced/search!export" /></td>
		</tr>
	</table>
	</div>
</s:form>
